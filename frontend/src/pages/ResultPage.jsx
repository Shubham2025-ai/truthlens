...
function scoreLabel(s) {
  return s >= 80 ? 'High Credibility' : s >= 60 ? 'Moderate Credibility' : s >= 40 ? 'Low Credibility' : 'Very Low Credibility'
}
function manipEmoji(l) {
...
.brand{font-size:28px;font-weight:800;color:#c0392b}.brand span{color:#1a1a1a}
...
.article-title{font-size:18px;font-weight:700;margin-bottom:8px;line-height:1.4}
...
<div class="header">
  <div><div class="brand">Truth<span>Lens</span></div><div style="font-size:12px;color:#888;margin-top:4px;">AI-powered news bias & credibility report</div></div>
  <div class="report-meta"><div>Generated: ${now}</div><div>Source: ${data.source || 'Unknown'}</div>${data.conflict_region ? `<div>Region: ${data.conflict_region}</div>` : ''}</div>
</div>
<div class="article-box">
  ...
  <div class="article-meta">
    ...
    ${data.url && data.url !== 'text-input' ? `<span><a href="${data.url}" style="color:#c0392b;">🔗 Original article</a></span>` : ''}
  </div>
</div>
<div class="score-row">
  <div class="score-card">
    ...
    <div style="font-size:13px;font-weight:600;color:${color};margin-top:6px;">${scoreLabel(score)}</div>
  </div>
  <div class="score-card" style="flex:2;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin:8px 0 6px;">
      <div style="font-size:20px;font-weight:700;">${bias.label || 'Unknown'}</div>
...
<div class="footer"><span>TruthLens — AI-Powered News Verification</span><span>AI-generated — not ground truth</span></div>
</div></body></html>`
...
export default function ResultPage() {
  ...
  const shareLink = async () => {
    // If we have an analysis id from Supabase, build a shareable URL
    const id = data?.id
    const shareUrl = id
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={shareLink}
                className="flex items-center gap-2 border border-white/15 hover:border-white/30 text-white/60 hover:text-white text-sm px-4 py-2 rounded-xl transition-all active:scale-95">
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {data.from_cache && (
                  <div className="inline-flex items-center gap-1.5 bg-blue-500/10 border border-blue-500/20 rounded-full px-3 py-1">
                    ...
                    <span className="text-xs font-mono text-blue-400">Cached</span>
                  </div>
                )}
                {data.scrape_failed && (
                  <div className="inline-flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/20 rounded-full px-3 py-1">
                    ...
                    <span className="text-xs font-mono text-amber-400">Source reputation only</span>
                  </div>
                )}
              </div>
        ...
        >
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => navigate('/compare')}
              className="border border-white/15 hover:border-white/30 text-white/60 hover:text-white text-sm px-5 py-2.5 rounded-xl transition-all">
              Compare sources
            </button>
          </div>
        </motion.div>
